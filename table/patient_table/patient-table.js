...
fetch('table/patient_table/patient-table.html')
  .then(res => res.text())
  .then(data => {
...
window.updatePatientTable = function() {
  ...
  patients.forEach((patient) => {
    ...
    row.innerHTML = `
      ...
      <td>
        <button class="action-btn" onclick="showPatientActions(${patient.id})">⋮</button>
      </td>
    `;
...
// Function to handle patient actions (edit/delete)
window.showPatientActions = function(patientId) {
  let patients = JSON.parse(localStorage.getItem('patients')) || [];
  const patient = patients.find(p => p.id === patientId);
  
  if (!patient) return;
  
  const action = confirm(`Patient: ${patient.firstName} ${patient.lastName}\n\nClick OK to Edit or Cancel to Delete`);
